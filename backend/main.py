...
def register(data: Register):
    ...
    conn.execute("INSERT INTO users(phone,password,name,stars,created_at) VALUES(?,?,?,?,?)",
                 (phone, password_hash(data.password), name, 0, datetime.utcnow().isoformat()))
...
def login(data: Login):
    conn = db()
    user = conn.execute("SELECT id,phone,name,stars FROM users WHERE phone=? AND password=?",
                        (data.phone.strip(), password_hash(data.password))).fetchone()
    if not user:
...
# Direct Stars injection has intentionally been removed. Stars are credited only
# after a verified Superme order in /superme/orders/verify.
@app.post("/stars/add")
...
def buy_gift(token: str, data: GiftBuy):
    ...
    # Keep a purchase record when the Superme commerce tables are available.
    try:
        conn.execute("INSERT INTO purchased_gifts(user_id,gift_id,gift_name,price_stars,purchased_at) VALUES(?,?,?,?,?)",
                     (user["id"], gift["id"], gift["name"], gift["price"], datetime.utcnow().isoformat()))
        conn.execute("INSERT INTO superme_transactions(user_id,kind,stars_delta,reference,created_at) VALUES(?,?,?,?,?)",
                     (user["id"], "gift_purchase", -gift["price"], str(gift["id"]), datetime.utcnow().isoformat()))
...
# Superme commerce is a separate router so the Telegram-style Android UI can
# use the same backend without importing the demo frontend from this repository.
